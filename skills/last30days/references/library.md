...
`"$RUN_SH" library feed` builds a local `index.html` and Atom `feed.xml` from saved briefs — offline, no network. Relay the printed paths. That is the whole request unless the user explicitly asks to publish.
