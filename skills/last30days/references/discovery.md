...
All three commands must share one save directory — run.py pins `LAST30DAYS_MEMORY_DIR`, so just don't override `--save-dir` mid-protocol. The handoff files expire after an hour; finish the protocol in the same session.

**1. Nominate** (timeout 3 min):

```bash
uv run --no-cache "$RUN_PY" --discover --nominate-only            # or: --discover "AI agents" --nominate-only
```
...
```bash
uv run --no-cache "$RUN_PY" --discover --judgments - <<'EOF'
...
```bash
uv run --no-cache "$RUN_PY" --discover --finalize --emit=compact --angles - <<'EOF'
