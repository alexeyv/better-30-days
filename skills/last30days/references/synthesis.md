...
- `@grok` is X's AI, not a person - cite it as "per Grok's AI analysis", never as community sentiment.
- YouTube items carry transcript highlights (quote, attribute to the channel) and top comments with like counts (quote, attribute to the commenter) - complementary signals, use both when present.
...
- A cluster about a different entity that shares the topic's name (another person named Michael, another product called X) is not evidence, whatever the engine scored it; drop it silently. Don't confuse this with the ClawdBot rule above: surprising facts about the same entity get reported; a different entity behind the same name gets discarded.
- If the community evidence is absent - the engine says `Nothing solid this window`, or the clusters were all wrong-entity or otherwise unusable - do not apologize and stop, and do not present a body built only from web supplements as if the footer's community counts backed it. Comments and stats attached to rejected clusters are not evidence: don't quote them or count them toward the two-comment rule. Extract every usable concrete claim from the web supplements and any weak clusters, open the body with `Recent community signal is thin; here is what is still visible:`, and follow with 3-5 specific bullets (each a real claim with a citation, not a caveat). Keep the footer and invitation. Never produce a pure apology report.
...
5. **Citations**: cite sparingly — one source per claim, the strongest one; prefer people over publications (an X post over the article covering it). No engagement numbers inside citations - the footer carries them. Link style `inline`: wrap cited handles/subs/publications as `[name](url)` with the URL copied from the evidence (a comment's URL exactly as given — never reconstruct one; no URL in the data → plain label, never an empty link). Link style `plain`: bare labels (`per @handle`, `per r/sub`); the footer and raw file carry URLs.
...
`What I learned:` is the literal first body line — no topic title, no `##` headers anywhere in the body. Every paragraph opens with a bold headline phrase, then ` - `. Headlines are specific ("BULLY dropped and it's dominating"), not generic ("Album release"). If positioning was resolved for a company/product and this month's evidence directly supports or contradicts a specific first-party claim, say so in one such paragraph anchored to the top item; if the evidence is merely adjacent to the pitch, say nothing about it. Keep that verdict windowed ("this month's conversation" - no trend verbs like "losing the narrative" that one 30-day window cannot support), and test only specific claims - never grade a broad tagline against a single thread. If the engine output contains a `**🔍 Research Coverage:**` block or a `## Pre-Research Status` block, include it verbatim between the body and the footer.

## RECOMMENDATIONS body

Rank by signal quality, not mention count. Weigh: practitioner testimony and expert switching > measurable claims > reasoned comparisons > mere convergence > descriptive mentions; promotional/course content counts for nothing. A quote calling the popular option's mention count a bias (training-data default, bootcamp inertia) is anti-signal: surface it and let it demote that pick. Independent replies to a "what's the best X?" thread converging on one name are the strongest endorsement - lead with them. Lead with the 30-day movement (who switched and why), not the status-quo leader. Split "best for X / best for Y" when the evidence supports different winners.
